...
function refreshDiplo(){
  ...
  if(rel==='neutral'){
    mk(`⚔ Объявить войну (${POLIT_WAR}🏛)`,'war',()=>declareWar(target));
    mk(`🤝 Заключить союз (${POLIT_ALLY}🏛)`,'ally',()=>formAlliance(target));
    mk('💰 Отправить поддержку','sup',()=>sendSupport(target));
  } else if(rel==='war'){
    mk(`🕊 Заключить мир (${POLIT_PEACE}🏛)`,'peace',()=>openPeaceDialog(target));
  } else if(rel==='ally'){
    mk(`⚔ Война + разрыв союза (${POLIT_WAR}🏛)`,'war',()=>{if(!politEnough(POLIT_WAR))return;setRelation(PLAYER,target,'neutral');declareWar(target);});
    mk(`💔 Расторгнуть союз (${POLIT_BREAK}🏛)`,'neutral',()=>breakAlliance(target));
    mk('💰 Отправить поддержку','sup',()=>sendSupport(target));
  }
}
