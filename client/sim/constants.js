/* СКОПИРОВАНО из server/sim/ скриптом scripts/sync-sim.js — НЕ РЕДАКТИРОВАТЬ. Источник: server/sim/constants.js */
...
module.exports = {
  ...
  VISION_SQUAD: 6, VISION_SHIP: 8, VISION_PLANE: 12, VISION_TOWER: 9, VISION_COAST: 7, VISION_COAST_SEARCH: 3, VISION_REFRESH: 0.4,
  ...
  SHIP_CAST_TIME: 2.5,    // сек постройки корабля у кромки воды перед посадкой
  SEA_MULT: 1.4,          // множитель скорости отряда, пока он на корабле (по воде)
  SEA_HYST: 2,            // тиков подряд над сушей для высадки (антидребезг у берега)
  SEA_MIN_OPEN: 1.5,      // мин. длина ОТКРЫТОЙ воды впереди (ед.), чтобы садиться на корабль (отсекает реки/мосты)
  SHIP_BOARD_PER: 0.06,   // сек посадки на 1 бойца — окно «каста» растёт с размером армии (успеть погрузиться до отплытия)
  ...
  FIELD_RANGE: 3.0,        // радиус ПРЕФИЛЬТРА spatial-grid в полевом бою (не дистанция сцепки)
  FIELD_CONTACT: 1.2,      // дорожная дистанция сцепки. 0.6 было слишком узко: армии, идущие в одну сторону по одной
                           //   дороге, застревали в зазоре 0.66–0.85 и НЕ дрались (визуально впритык, но мимо). Колонны юнитов
                           //   имеют длину ~1–1.5, поэтому «фронты касаются» при дистанции центров ~1.2. При 1.2 «касающихся, но
                           //   не дерущихся» пар не остаётся (замерено). Окно ещё растёт со скоростью×dt (страховка от проскока).
  UNIT_MIN: 0.5,           // отряд «жив»/бьёт при fcount ≥ этого (полевой бой/осада)
  // 👥 типы юнитов: гарнизон/отряд хранит СОСТАВ {inf,arc,cav}; units/fcount — сумма (совместимость).
  //    Отправка/урон/слияние — пропорционально по типам.
  UNIT_TYPES: ['inf', 'arc', 'cav'],
  START_COMP: { inf: 0.7, arc: 0.2, cav: 0.1 },   // стартовый состав гарнизонов
  // ⚔ БОЙ ТИПОВ: треугольник контрбонусов inf›cav›arc›inf (пехота бьёт конницу, конница — лучников, лучники — пехоту).
  //   Сила = UNIT_COUNTER_BONUS (доля): 0 = типы в бою НЕ различаются (идентично прежнему поведению);
  //   напр. 0.5 = атакующий состав наносит ×(1+0.5) по контримому типу и ×(1−0.5) по контрящему (взвешенно по долям).
  //   Действует и в полевом бою, и в осаде (Sim.fieldBattles / City siege). Тюнится через balance (колонка unit_counter_bonus).
  UNIT_COUNTER_BONUS: 0,
