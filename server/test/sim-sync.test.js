// GUARD: браузерная копия симулятора client/sim/ обязана совпадать с
...
group('Браузерная копия симулятора (server/sim ⟷ client/sim)');
...
for (const f of FILES) {
  test(`sim/${f} синхронен с server/sim/${f}`, () => {
    ...
    assert(fs.readFileSync(p, 'utf8') === content(f),
      `sim/${f} устарел → запусти \`node scripts/sync-sim.js\` и закоммить client/sim/`);
  });
}
